...
var StreamBuffer_1 = require("./util/StreamBuffer");
...
var HttpProxy = (function () {
    ...
    HttpProxy.prototype.patchServerExport = function (agent, httpModule) {
        var originalServer = httpModule.Server;
        var self = this;
        httpModule.Server = function glimpseHttpProxyServer() {
            // NOTE: https.Server() and http.Server() have different signatures:
            ...
            //
            //       Note that we do *not* pass the callback into the original Server();
            ...
            //
            // TODO: We should also patch the equivalent function `addListener()`.
            var oldOn = server.on;
            server.on = function glimpseHttpProxyOn(eventName, listener) {
                var onArgs = [];
                ...
                if (eventName === 'request') {
                    ...
                    listener = function glimpseHttpProxyOnListener(req, res) {
        ...
        httpModule.createServer = function glimpseServerCreateServer() {
            var args = [];
            for (var _i = 0; _i < arguments.length; _i++) {
                args[_i] = arguments[_i];
            }
            return (_a = httpModule.Server).call.apply(_a, [this].concat(args));
            var _a;
        };
    };
    HttpProxy.prototype.setupServerProxy = function (agent, cb) {
        ...
        return function internalCallback(req, res) {
            ...
            var _shouldInjectScript;
            var shouldBufferStream = true;
            var contentLengthHeader = 'Content-Length';
            var etagHeader = 'Etag';
            var shouldInjectScript = function () {
                if (_shouldInjectScript === undefined) {
                    _shouldInjectScript = false;
                    // TODO: This mount point will likely become configurable (so pull from configuration).
                    if (req.url.indexOf('/glimpse') !== 0) {
                        var contentType = HttpHelper_1.HttpHelper.parseContentType(res.getHeader('Content-Type'));
                        var contentEncoding = res.getHeader('Content-Encoding');
                        // Only 'text/html' or '' (i.e. not specified) are valid content types.
                        var isValidContentType = (contentType.type === 'text' && contentType.subtype === 'html') || (contentType.type === '' && contentType.subtype === '');
                        // Only 'identity' or undefined (i.e. not specified) are valid content encodings.
                        var isValidContentEncoding = contentEncoding === undefined || contentEncoding === '' || contentEncoding.toLowerCase() === 'identity';
                        if (isValidContentType && isValidContentEncoding) {
                            // NOTE: If headers have been sent (i.e. res.writeHead() called), we can't inject script because we can't update the Content-Length or Etag headers.
                            // TODO: Log Glimpse error/warning about use of res.writeHead()? 
                            if (!res.headersSent || (res.getHeader(contentLengthHeader) === undefined && res.getHeader(etagHeader) === undefined)) {
                                _shouldInjectScript = true;
                            }
                        }
                    }
                }
                return _shouldInjectScript;
            };
            var streamBuffer = new StreamBuffer_1.default();
            var responseBodyChunks = [];
            var responseBodyLength = 0;
            var bufferResponseBody = function (chunk) {
                if ((typeof chunk === 'string' || Buffer.isBuffer(chunk)) && chunk.length) {
            ...
            res.write = function glimpseHttpProxyWrite(chunk, encoding, callback) {
                var writeArgs = [];
                for (var _i = 3; _i < arguments.length; _i++) {
                    writeArgs[_i - 3] = arguments[_i];
                }
                if (!responseStartTime) {
                    responseStartTime = self.getCurrentTimeStamp();
                    self.raiseResponseStartEvent(req, res, responseStartTime);
                }
                // TODO: Re-enable this check when we have an effective context tracking implementation in place
                //agent.providers.contextManager.checkContextID('HttpProxy::(response.write())', context.id);
                if (shouldInjectScript()) {
                    if (shouldBufferStream && (typeof chunk === 'string' || Buffer.isBuffer(chunk))) {
                        streamBuffer.write(chunk, typeof encoding === 'string' ? encoding : undefined);
                        // If encoding was omitted, the callback is actually the second argument... 
                        if (typeof encoding === 'function') {
                            callback = encoding;
                        }
                        // Invoke any callback as the content has been "written"...
                        if (typeof callback === 'function') {
                            callback();
                        }
                        // Return `true` as we've buffered the content...
                        return true;
                    }
                }
                else {
                    // Only capture the response body if we're not injecting script (as that's captured elsewhere)...
                    bufferResponseBody(chunk);
                }
                return originalWrite.call.apply(originalWrite, [this, chunk, encoding, callback].concat(writeArgs));
            };
            var originalEnd = res.end;
            res.end = function glimpseHttpProxyEnd(chunk, encoding) {
                var endArgs = [];
                for (var _i = 2; _i < arguments.length; _i++) {
                    endArgs[_i - 2] = arguments[_i];
                }
                if (shouldInjectScript()) {
                    // If `end()` was called with a final chunk, buffer it...
                    if (typeof chunk === 'string' || Buffer.isBuffer(chunk)) {
                        streamBuffer.write(chunk, typeof encoding === 'string' ? encoding : undefined);
                    }
                    shouldBufferStream = false;
                    // Perform the injection...
                    var scripts = self.scriptManager.getScriptTagsForCurrentRequest(context);
                    var bufferedChunk = new Buffer(self.scriptManager.injectScript(streamBuffer.toString(), scripts), streamBuffer.encoding);
                    // Capture the injected response body...
                    bufferResponseBody(bufferedChunk);
                    // If there's an existing `Content-Length` header, update it with the new injected size...
                    if (res.getHeader(contentLengthHeader)) {
                        res.removeHeader(contentLengthHeader);
                        res.setHeader(contentLengthHeader, bufferedChunk.length.toString());
                    }
                    // If there's an existing `Etag` header, remove it (to prevent caching injected script content)...
                    if (res.getHeader(etagHeader)) {
                        res.removeHeader(etagHeader);
                    }
                    if (typeof chunk === 'undefined' || typeof chunk === 'function') {
                        // NOTE: `end()` implicitly passes its final chunk to `write()`.
                        //       Write can be patched (by us and others) to monitor its use.
                        //       We need to write the entire buffered content in `end()`.
                        //       If we passed buffer to `end()`, which is then passed to `write()`,
                        //       it may cause those monitors to report repeated writes.
                        //
                        //       Hence, if there was no final chunk, we push the buffer to the 
                        //       original `write()` instead. Otherwise, if there was a final chunk,
                        //       we pass it to the original `end()`, to be passed through to the 
                        //       latest `write()`, as there should be one more write recorded anyway.
                        //        
                        originalWrite.call(this, bufferedChunk);
                        return originalEnd.call.apply(originalEnd, [this, chunk, encoding].concat(endArgs));
                    }
                    else if (typeof encoding === 'function') {
                        // The encoding wasn't specified (i.e. it's actually the callback), so keep it in the argument list...
                        return originalEnd.call.apply(originalEnd, [this, bufferedChunk, encoding].concat(endArgs));
                    }
                    else {
                        // The encoding was specified but is now not needed, as we're writing an opaque Buffer...
                        return originalEnd.call.apply(originalEnd, [this, bufferedChunk].concat(endArgs));
                    }
                }
                return originalEnd.call.apply(originalEnd, [this, chunk, encoding].concat(endArgs));
            };
            ...
            res.on('finish', function () {
                ...
                if (!requestEndSent) {
                    ...
                    // the body has not been received, but we try and send
                    // whatever body we have received so far.
                    self.raiseRequestEndEvent(req, res, requestBodyChunks, requestBodyLength, requestStartTime, multiPartFormSummarizer ? multiPartFormSummarizer.getParts() : []);
                    requestEndSent = true;
    ...
    HttpProxy.prototype.init = function (agent, httpModule, resolvedModulePath, errorReportingService) {
        ...
        this.scriptManager = agent.providers.scriptManager;
        ...
        this.patchServerExport(agent, httpModule);
