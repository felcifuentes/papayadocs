...
var ScriptManager = (function () {
    function ScriptManager(contextManager, resourceProvider, configSettings) {
    ...
    ScriptManager.prototype.hudScript = function (context, resources) {
        var uri = '<script src="%s" id="__glimpse_hud" data-request-id="%s" data-client-template="%s" data-context-template="%s" data-context-summary-template="%s" data-metadata-template="%s"></script>' + os_1.EOL;
        return util.format(uri, resources[ScriptManager.hudScriptTemplate], this.getCurrentContextId(context), resources[ScriptManager.clientScriptTemplate], resources[ScriptManager.contextTemplate], resources[ScriptManager.contextSummaryTemplate], resources[ScriptManager.metadataTemplate]);
    };
    ;
    ScriptManager.prototype.agentScript = function (context, resources) {
        ...
        var settings = escapeHtml(JSON.stringify(settingsObject, undefined, 0));
        var uri = "<script src=\"" + resources[ScriptManager.browserAgentScriptTemplate] + "\" id=\"__glimpse_browser_agent\" data-request-id=\"" + this.getCurrentContextId(context) + "\" config-settings=\"" + settings + "\" data-message-ingress-template=\"" + resources[ScriptManager.messageIngressTemplate] + "\"></script>" + os_1.EOL;
        return uri;
    };
    ;
    ...
    ScriptManager.contextSummaryTemplate = 'contextSummaryTemplate';
