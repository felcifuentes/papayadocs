        ...
        function asyncTrackRequire(moduleName) {
            ...
            if (cbConfig[moduleName] && cbConfig.hasOwnProperty(moduleName)) {
                ...
                if (loadedModule.setScheduler && !loadedModule.setScheduler.__asyncTrack_original__ && typeof loadedModule.setScheduler === 'function') {
                    var originalScheduler_1 = loadedModule.setScheduler(function (fn) {
                        if (fn && typeof fn === 'function') {
                            ...
                            var wrapped = self.wrapCallback(fn, undefined, asyncId, asyncState, ['bluebird', 'async', 'prototype', 'schedule']);
                            arguments[0] = wrapped;
                        }
                        originalScheduler_1.apply(this, arguments);
                    });
                }
            }
            else if (moduleName === 'redis') {
                var oldSend_1 = loadedModule.RedisClient.prototype.internal_send_command;
                loadedModule.RedisClient.prototype.internal_send_command = function (command) {
                    var rest = [];
                    ...
                    if (command && command.callback) {
                        ...
                        var asyncId = self.getNextAsyncId();
                        var asyncState = self.raiseAsyncTransition(self.currentAsyncId, asyncId);
                        var wrapped = self.wrapCallback(callback, undefined, asyncId, asyncState, ['redis', 'RedisClient', 'prototype', 'internal_send_command']);
                        command.callback = wrapped;
                    }
                    return oldSend_1.call.apply(oldSend_1, [this, command].concat(rest));
                };
            }
            return loadedModule;
        }
            ...
            for (var prop in config) {
                if (config.hasOwnProperty(prop)) {
                    if (prop !== '__returnVal__' && prop !== '__configType__' &&
                        ...
                        var newPath = curPath.slice(0).concat(prop);
                        if (config[prop].__configType__ === 'package') {
                            ...
                            var instrumentedValue = this.instruCbReg(config, prop, pkg[prop], newPath);
                            var propDescriptor = Object.getOwnPropertyDescriptor(pkg, prop);
                            // instrument the method in the pkg
                            if (propDescriptor && propDescriptor.writable) {
                                pkg[prop] = instrumentedValue;
                            }
                            else {
                                Object.defineProperty(pkg, prop, __assign({}, propDescriptor, { writable: true, value: instrumentedValue }));
                            }
