...
export class ModuleManager implements IModuleManager {
    ...
    private instrumentedModules: { [path: string]: any } = {}; // tslint:disable-line:no-any
    ...
    public init(agent: IAgent, moduleModule) {
        ...
        this.nonExperimentalModules.set('body-parser', true);
        this.nonExperimentalModules.set('cookie-parser', true);
        this.nonExperimentalModules.set('morgan', true);
        this.nonExperimentalModules.set('serve-favicon', true);
        this.nonExperimentalModules.set('serve-index', true);
        this.nonExperimentalModules.set('serve-static', true);
        this.nonExperimentalModules.set('express-session', true);
        this.nonExperimentalModules.set('debug', true);
        this.nonExperimentalModules.set('compression', true);
        this.nonExperimentalModules.set('redis', true);
        ...
        const glimpseRequire = function glimpseRequire(moduleId) {
            ...
            let managedModule = self.instrumentedModules[modulePath] || originalModule;
            ...
            if (self.moduleInstrumentors[moduleId] && isEnabled) {
                ...
                const moduleInfo: IModuleInfo = getModuleInfo(moduleId, modulePath, managedModule);
                ...
                for (const instrumentorEntry of self.moduleInstrumentors[moduleId]) {
                    ...
                    if (satisfies(moduleInfo.version, supportedVersion)) {
                        if (!instrumentorEntry.enabledModules[modulePath]) {
                            ...
                            // If instrumentation was enabled and a new module (i.e. set of exports) generated,
                            // use that going forward...
                            if (result.isEnabled && result.newModule) {
                                self.instrumentedModules[modulePath] = managedModule = result.newModule;
                            }
    ...
    public isModuleInstrumented(moduleId: string): boolean {
        return !!this.moduleInstrumentors[moduleId];
    }
