...
export class RequireAnalyzer {
                ...
                if (packageName === RequireAnalyzer.applicationPackageName) {
                    ...
                    for (let i = 0; i < knownModules.length; i++) {
                        ...
                        if (parentModule.match) {
                            if (parentModule.match.test(nodeModule.filename)) {
    ...
    public static getGlimpseProductPackages(): IKnownPackage[] {
        const glimpseRoot = path.join(__dirname, '../../../').replace(/\\/g, '\\\\').replace(/\//g, '\\/');
        const excludes = [
            {
                packageName: '@glimpse/glimpse',
                match: new RegExp(`^${glimpseRoot}glimpse\\${path.sep}index.js`)
            },
            {
                packageName: '@glimpse/glimpse-agent-node',
                match: new RegExp(`^${glimpseRoot}glimpse.agent\\${path.sep}release\\${path.sep}.*`)
            },
            {
                packageName: '@glimpse/glimpse-server',
                match: new RegExp(`^${glimpseRoot}glimpse.server\\${path.sep}release\\${path.sep}.*`)
            },
            {
                packageName: '@glimpse/glimpse-common',
                match: new RegExp(`^${glimpseRoot}glimpse.common\\${path.sep}release\\${path.sep}.*`)
            }
        ];
        return excludes;
    }
