...
export class MiddlewareWrapper {
    ...
    private wrapErrorMiddleware(
        ...
        registrationCallstack: IStackFrame[]): IMiddlewareErrorFunction & IWrappedMiddlewareFunction {
        const self = this;
        const wrappedMiddleware = <IMiddlewareErrorFunction & IWrappedMiddlewareFunction>function wrappedMiddleware(err, req, res, next) {
    ...
    private wrapMiddleware(
        ...
        method: string,
        originalMiddleware: IMiddlewareFunction,
        registrationCallstack: IStackFrame[]): IMiddlewareFunction & IWrappedMiddlewareFunction {
        const self = this;
        const wrappedMiddleware = <IMiddlewareFunction & IWrappedMiddlewareFunction >function wrappedMiddleware(req, res, next) {
            self.wrapCommonMiddleware(
                paths,
                method,
                originalMiddleware.name,
    ...
    public wrap(
        ...
        registrationCallstack: IStackFrame[]): IMiddlewareFunction | IMiddlewareErrorFunction {

        if (originalMiddleware && typeof originalMiddleware === 'function') {
            let newMiddleware: (IMiddlewareFunction | IMiddlewareErrorFunction) & IWrappedMiddlewareFunction;

            if (originalMiddleware.length === 2 || originalMiddleware.length === 3) {
                newMiddleware = this.wrapMiddleware(paths, method, <IMiddlewareFunction>originalMiddleware, registrationCallstack);
            }
            else if (originalMiddleware.length === 4) {
                newMiddleware = this.wrapErrorMiddleware(paths, method, <IMiddlewareErrorFunction>originalMiddleware, registrationCallstack);
            }

            if (newMiddleware) {
                newMiddleware.__glimpse_original = originalMiddleware;
                newMiddleware.glimpse = {
