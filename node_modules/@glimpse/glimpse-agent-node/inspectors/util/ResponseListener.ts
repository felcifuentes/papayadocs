...
export class ResponseListener extends events.EventEmitter {
    ...
    private static addFormattedHeaders(
    ...
    private patchResponseEnd(res: http.ServerResponse) {
        const oldEnd = res.end;
        const self = this;
        res.end = function newEnd() {
            const result = oldEnd.apply(this, arguments);

            // on node version 8.x, end() will call an internal method for write, so our write proxy doesn't
            // get hit.  We remedy that here.
