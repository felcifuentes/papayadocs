...
export class MongoDBInspector {
    ...
    private sendReadStartMessage(
        ...
        if (context) {
            ...
            this.agent.broker.createAndSendMessage(
                ...
                // OK for now to send  *-end messages with this "read-start".  This is the message associated with the "end" of the
                // of the find() call, so it is really an "end message".  Name needs cleaned up.
                [
    ...
    private insertMethodsInspector(event: IInstrumentationEvent) {
        const connectionInfo = MongoDBInspector.GetConnectionInfoFromCollection(
            event.data.originalThis
        );
    ...
    private countInspector(event: IInstrumentationEvent) {
        ...
        const startTime = DateTimeValue.fromMomentAndHRTime(
    ...
    private readStartInspector(event: IInstrumentationEvent) {
        ...
        if (!cursorCustomOpts.getMessageSent('data-store-mongodb-read-start')) {
            cursorCustomOpts.setMessageSent('data-store-mongodb-read-start', true);

            const connectionInfo: IConnectionInfo = MongoDBInspector.GetConnectionInfoFromCursor(
                originalThis
            );

            cursorCustomOpts.operationName = event.data.methodName;
            cursorCustomOpts.readStartTime = DateTimeValue.fromUnixMillisecondTimestamp(
                event.timestamp,
                event.time
            );
    ...
    private readEndInspector(event: IInstrumentationEvent) {
        ...
        if (!cursorCustomOpts.getMessageSent('data-store-mongodb-read-end')) {
            cursorCustomOpts.setMessageSent('data-store-mongodb-read-end', true);
