...
import {
    ...
    EVENT_HTTP_CLIENT_REQUEST_CLOSE,
    ...
    EVENT_HTTP_CLIENT_RESPONSE_ABORTED,
    EVENT_HTTP_CLIENT_RESPONSE_END,
...
export class ClientRequestInspector {
    ...
    public after(masterData: IRequestData): void {
        if (masterData.context && masterData.responseMessage) {
            ...
            const index = {
                ...
                method: masterData.response.method
            };

            masterData.responseMessage.indices = index;
    ...
    public init(agent: IAgent, errorReportingService: IErrorReportingService) {
        ...
        this.listeners = {
            ...
            [EVENT_HTTP_CLIENT_REQUEST_CLOSE]: (event) => this.onRequestClose(event),
            ...
            [EVENT_HTTP_CLIENT_RESPONSE_ABORTED]: (event) => this.onResponseDone(event),
            [EVENT_HTTP_CLIENT_RESPONSE_END]: (event) => this.onResponseDone(event),
    ...
    private onRequestEnd(event: IInstrumentationEvent): void {
        ...
        const masterData = this.requests[eventData.id];
        if (!masterData) {
            // TODO:  this is possible in the event response.end() is called before request.end()
            //        ideally, we could display some kind of warning here, but it isn't actionable.
        }
        else {
    ...
    private onRequestClose(event: IInstrumentationEvent): void {
        // Ensure we've published the response end event (if for some reason, no response end/aborted event was sent)...
    ...
    private onResponseDone(event: IInstrumentationEvent): void {
        const eventData: IClientRequestEvent = event.data;
        const masterData = this.requests[eventData.id];
        if (masterData) {
            if (!masterData.requestStartTime) {
                // TODO: User called response.end() before request.end() was called.
                //       Ideally, we'd have some kind of warning here, but not sure it is actionable.
            }
            else if (masterData.context) {
