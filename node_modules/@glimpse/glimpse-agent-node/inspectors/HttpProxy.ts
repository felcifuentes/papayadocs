...
import StreamBuffer from './util/StreamBuffer';
...
export class HttpProxy {
    ...
    private patchServerExport(agent: IAgent, httpModule) {
        const originalServer = httpModule.Server;
        const self = this;
        httpModule.Server = function glimpseHttpProxyServer(...args) {
            ...
            server.on = function glimpseHttpProxyOn(eventName: string, listener, ...onArgs) {
        ...
        httpModule.createServer = function glimpseServerCreateServer(...args) {
            return httpModule.Server.call(this, ...args);
        };
    }
    ...
    private setupServerProxy(agent: IAgent, cb) {
        ...
        return function internalCallback(req: http.IncomingMessage, res: http.ServerResponse, ...rest) {
            ...
            let _shouldInjectScript: boolean;
            let shouldBufferStream = true;
            ...
            const shouldInjectScript = () => {
                if (_shouldInjectScript === undefined) {
                    ...
                    if (req.url.indexOf('/glimpse') !== 0) {
                        const contentType = HttpHelper.parseContentType(res.getHeader('Content-Type'));
                        const contentEncoding = res.getHeader('Content-Encoding');

                        // Only 'text/html' or '' (i.e. not specified) are valid content types.
                        const isValidContentType = (contentType.type === 'text' && contentType.subtype === 'html') || (contentType.type === '' && contentType.subtype === '');

                        // Only 'identity' or undefined (i.e. not specified) are valid content encodings.
                        const isValidContentEncoding = contentEncoding === undefined || contentEncoding === '' || contentEncoding.toLowerCase() === 'identity';
            ...
            const bufferResponseBody = chunk => {
            ...
            res.write = function glimpseHttpProxyWrite(chunk, encoding?, callback?, ...writeArgs): boolean {
                ...
                return originalWrite.call(this, chunk, encoding, callback, ...writeArgs);
            };

            const originalEnd = res.end;
            res.end = function glimpseHttpProxyEnd(chunk?, encoding?, ...endArgs) {
                if (shouldInjectScript()) {
                    ...
                    const scripts = self.scriptManager.getScriptTagsForCurrentRequest(context);
                    const bufferedChunk = new Buffer(self.scriptManager.injectScript(streamBuffer.toString(), scripts), streamBuffer.encoding);
