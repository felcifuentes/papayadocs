...
import { PackageHelper, IConfigSettings } from '@glimpse/glimpse-common';
...
export class ScriptManager implements IScriptManager {
    ...
    private static contextSummaryTemplate = 'contextSummaryTemplate';
    ...
    public constructor(private contextManager: IContextManager, private resourceProvider: IResourceProvider, private configSettings: IConfigSettings) {
    ...
    public injectScript(htmlbody, payload: string): string {
    ...
    private hudScript(context: IContext, resources: { [key: string]: string }) {
        const uri = '<script src="%s" id="__glimpse_hud" data-request-id="%s" data-client-template="%s" data-context-template="%s" data-context-summary-template="%s" data-metadata-template="%s"></script>' + EOL;
        ...
        return util.format(
            ...
            resources[ScriptManager.contextSummaryTemplate],
            resources[ScriptManager.metadataTemplate]);
    };

    private agentScript(context: IContext, resources: { [key: string]: string }) {
        const settingsObject = {
            'inspector.http.body.capture.maxsize': this.configSettings.get('inspector.http.body.capture.maxsize', 132000),
            'server.max.json.body.size': this.configSettings.get('server.max.json.body.size', 135000)
        };
        const settings = escapeHtml(JSON.stringify(settingsObject, undefined, 0));

        const uri = `<script src="${resources[ScriptManager.browserAgentScriptTemplate]}" id="__glimpse_browser_agent" data-request-id="${this.getCurrentContextId(context)}" config-settings="${settings}" data-message-ingress-template="${resources[ScriptManager.messageIngressTemplate]}"></script>` + EOL;
        return uri;
    };

    public getScriptTagsForCurrentRequest(context: IContext): string {
